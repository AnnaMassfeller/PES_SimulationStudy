...
E.act <- function(dc=0.95, x=11/10, Cl=100, Ch=10, v=0.95) {
	
	x*Cl+100/209*(t.fix(dc,v)+1)*Ch
...
#results
test.batch.act <- batch.act()
test.batch.opt <- batch.opt()

test.multi.patch.batch.act<- multi.patch.batch(test.batch.act, method = "select")
test.multi.patch.batch.opt<- multi.patch.batch(test.batch.opt, method = "select")
