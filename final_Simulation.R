#Final file to run simulation

#we start by running the oroginal code from Gibbons et al.(2011)
#then we introduce the assumed changes taht come with the use of a robot furtehr down


##############################
##original Gibbons approach##
#############################
...
B.act <- function(x=11/10, p0, a) {
 ...
 ifelse(x>11/10, L.func(1, p0, a), p0)
 
}

G.act <- function(x=11/10, p0, a) {
...
#replicate reuslts from Gibbons
#we only want test.batch.act and test batch.opt with varying values for d
#in order to reduce the computational power needed, we decrease the setps for other vars as follows

#the following two are to reproduce the resulst from Gibbons
test.batch.act_AM <- batch.act(dc = seq(0.1,0.9, by = 0.2),
                               a=c(0.6667, 1.0, 10.0), 
                               p0=seq(0.1, 0.9, by=0.1),
                               x.mult=seq(0, 50, by=5))





test.batch.opt_AM <- batch.opt(db = seq(0.1,0.9, by = 0.2),
                               a=c(0.6667, 1.0, 10.0), 
                               p0=seq(0.1, 0.9, by=0.1), 
                               x.mult=seq(0, 50, by=5))
...
test.batch.act_AM_robot<- test.batch.act_AM_robot_Lfunc
#test.batch.opt_AM_robot<- test.batch.opt_AM_robot_Lfunc_rangea


#bind three version together
test.batch.opt_AM_robot <- rbind(test.batch.opt_AM_robot_Lfunc_0.667,test.batch.opt_AM_robot_Lfunc_1,test.batch.opt_AM_robot_Lfunc_10)
